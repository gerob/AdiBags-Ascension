...
do
	...
	-- Ascension Personal Bank (GuildBankFrame + GetGuildBank* APIs)
	local PERSONAL_BANK_CONTAINER = -99
	local PERSONAL_BANK = { [PERSONAL_BANK_CONTAINER] = PERSONAL_BANK_CONTAINER }
	...
	addon.PERSONAL_BANK_CONTAINER = PERSONAL_BANK_CONTAINER
	addon.MAX_GUILDBANK_SLOTS = 98 -- 7 columns x 14 slots
	addon.BAG_IDS = { BAGS = BAGS, BANK = BANK, PERSONAL_BANK = PERSONAL_BANK, ALL = ALL }
end
...
local DEFAULT_SETTINGS = {
	profile = {
		...
		positions = {
			...
			PersonalBank = { point = "TOPLEFT", xOffset = 32, yOffset = -104 },
		},
		scale = 0.8,
		rowWidth = { ['*'] = 9 },
		maxHeight = 0.60,
		...
		skin = {
			...
			BankColor = { 0, 0, 0.5, 1 },
			PersonalBankColor = { 0.25, 0.1, 0.35, 1 },
		},
	},
	char = {
		collapsedSections = {
...
do
	...
	function personalBank:PostEnable()
		self:RegisterMessage('AdiBags_InteractingWindowChanged')
		self:RegisterEvent('GUILDBANKBAGSLOTS_CHANGED', 'GuildBankContentChanged')
		self:RegisterEvent('GUILDBANK_ITEM_LOCK_CHANGED', 'GuildBankContentChanged')
		self:RegisterEvent('GUILDBANK_UPDATE_TABS', 'GuildBankTabsChanged')
	...
	function personalBank:OnAddonLoaded(_, name)
	...
	function personalBank:OnGuildBankClosed()
		...
		if self:IsOpen() and self.frame and self.frame:IsShown() then
			...
			self.frame:Hide()
			addon:SendMessage('AdiBags_BagClosed', self.bagName, self)
			self.closingGuildBank = false
		end
	end

	function personalBank:PostDisable()
		local frame = _G.GuildBankFrame
		if frame then
			RestoreStockGuildBankFrame(frame)
			if self.guildBankHooked and self.hooks and self.hooks[frame] then
				self.suppressGuildBankHide = true
				self.hooks[frame].Hide(frame)
				self.suppressGuildBankHide = false
			end
		end
		self.sessionOpen = false
	end
	...
	function personalBank:CanOpen()
		return self:IsEnabled() and (self.sessionOpen or addon:GetInteractingWindow() == "GUILDBANKFRAME")
	end

	function personalBank:PostClose()
		if self.closingGuildBank then return end
		if self.sessionOpen and _G.CloseGuildBankFrame then
			self.closingGuildBank = true
			local frame = _G.GuildBankFrame
			if frame then
				RestoreStockGuildBankFrame(frame)
	...
	function personalBank:GuildBankContentChanged()
		if self:IsOpen() then
			addon:SendMessage('AdiBags_BagUpdated', addon.PERSONAL_BANK_CONTAINER)
		end
	end
