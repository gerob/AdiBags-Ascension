...
--------------------------------------------------------------------------------
-- Personal / guild bank button sub-type
--------------------------------------------------------------------------------
...
function guildBankButtonProto:OnAcquire(container, bag, slot)
	...
	-- Template wires OnUpdate + UpdateTooltip to SetBagItem, which fails for
	-- guild/personal bank and flickers tooltips. Replace both.
	self:SetScript('OnUpdate', nil)
	self.UpdateTooltip = guildBankButtonProto.UpdateTooltip
end

function guildBankButtonProto:GetGuildTab()
	return GetCurrentGuildBankTab and GetCurrentGuildBankTab() or 1
end

function guildBankButtonProto:ShowTooltip()
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
	self:UpdateTooltip()
	GameTooltip:Show()
end

-- Called by GameTooltip_OnUpdate while owned; must use SetGuildBankItem, not SetBagItem.
function guildBankButtonProto:UpdateTooltip()
	...
	if GameTooltip.SetGuildBankItem then
		...
		if link then
			...
			GameTooltip:SetText(EMPTY or "Empty", 1, 1, 1)
		end
	end
end

function guildBankButtonProto:FullUpdate()
	local tab, slot = self:GetGuildTab(), self.slot
	local texture, count, locked = GetGuildBankItemInfo(tab, slot)
	local link = GetGuildBankItemLink(tab, slot)
	self.itemLink = link
	self.itemId = link and tonumber(link:match("item:(%d+)")) or nil
...
function guildBankButtonProto:GetCount()
	local _, count = GetGuildBankItemInfo(self:GetGuildTab(), self.slot)
	return count or 0
end

function guildBankButtonProto:IsLocked()
	local _, _, locked = GetGuildBankItemInfo(self:GetGuildTab(), self.slot)
	return locked
end

function guildBankButtonProto:IsBank()
	return true
end

function guildBankButtonProto:UpdateCooldown()
end

function guildBankButtonProto:UpdateBorder(isolatedEvent)
...
function guildBankButtonProto:OnClick(button)
	...
	if button == "RightButton" and AutoStoreGuildBankItem then
...
function guildBankButtonProto:OnDragStart()
	PickupGuildBankItem(self:GetGuildTab(), self.slot)
end

function guildBankButtonProto:OnReceiveDrag()
	PickupGuildBankItem(self:GetGuildTab(), self.slot)
end
...
local PERSONAL_BANK_BAG_IDS = addon.BAG_IDS.PERSONAL_BANK
function buttonProto:IsBank()
	return not not (BANK_BAG_IDS[self.bag] or PERSONAL_BANK_BAG_IDS[self.bag])
end
