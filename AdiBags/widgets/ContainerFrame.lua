...
local PERSONAL_BANK_CONTAINER = addon.PERSONAL_BANK_CONTAINER
local MAX_GUILDBANK_SLOTS = addon.MAX_GUILDBANK_SLOTS
...
function containerProto:OnCreate(name, bagIds, isBank)
	...
	self.bagIds = bagIds
	self.isBank = isBank
	self.isPersonalBank = (name == "PersonalBank")
	self.topPadding = self.isPersonalBank and (addon.TOP_PADDING + 8) or addon.TOP_PADDING
	...
	local headerTop = self.isPersonalBank and (BAG_INSET + 6) or BAG_INSET

	local headerLeftRegion = SimpleLayeredRegion:Create(self, "TOPLEFT", "RIGHT", 4)
	headerLeftRegion:SetPoint("TOPLEFT", BAG_INSET, -headerTop)
	self.HeaderLeftRegion = headerLeftRegion
	self:AddWidget(headerLeftRegion)
	headerLeftRegion:SetFrameLevel(minFrameLevel)

	-- Keep right header at BAG_INSET so module buttons (T/V/N) align with CloseButton
	...
	--------------------------------------------------------------------------------
		...
		AdiBagsBagMenu:SetFrameStrata(self:GetFrameStrata())
		AdiBagsBagMenu:SetFrameLevel(self:GetFrameLevel() + 5)
	...
	--------------------------------------------------------------------------------
		...
		-- set the frame strata to be higher than the title text's strata
		anchor:SetFrameStrata(self:GetFrameStrata())
		anchor:SetFrameLevel(self:GetFrameLevel() + 5)
	...
	content:SetPoint("TOPLEFT", BAG_INSET, -self.topPadding)
...
local function FindBagWithRoom(self, itemFamily)
	if self.isPersonalBank then
		return PERSONAL_BANK_CONTAINER
	end
	local fallback
	for bag in pairs(self.bagIds) do
...
do
	...
	FindFreeSlot = function(self, item)
		if self.isPersonalBank then
			...
			for slot = 1, MAX_GUILDBANK_SLOTS or 98 do
				local texture = GetGuildBankItemInfo(tab, slot)
				if not texture then
					return GetSlotId(PERSONAL_BANK_CONTAINER, slot)
				end
			end
			return
		end
...
function containerProto:OnClick(...)
	...
	if itemLink then
		...
		if slotId then
			if self.isPersonalBank then
				local _, slot = GetBagSlotFromId(slotId)
				local tab = GetCurrentGuildBankTab and GetCurrentGuildBankTab() or 1
...
function containerProto:OnLayout()
	...
	self:SetHeight(self.topPadding + BAG_INSET + bottomHeight + self.Content:GetHeight())
end

--------------------------------------------------------------------------------
-- Personal Bank tab strip
--------------------------------------------------------------------------------

local MAX_PERSONAL_BANK_TABS = 8
local TAB_SIZE = 32
local TAB_SPACING = 4
...
function containerProto:CreatePersonalBankTabBar()
	...
	tabBar:SetPoint("TOPLEFT", self, "TOPRIGHT", 2, -4)
	tabBar:SetPoint("BOTTOMLEFT", self, "BOTTOMRIGHT", 2, 4)
	self.PersonalBankTabBar = tabBar
	self.personalBankTabs = {}

	for i = 1, MAX_PERSONAL_BANK_TABS do
		local btn = CreateFrame("CheckButton", nil, tabBar)
		btn:SetSize(TAB_SIZE, TAB_SIZE)
		btn:SetPoint("TOPLEFT", 2, -((i - 1) * (TAB_SIZE + TAB_SPACING)))
		...
		btn:SetScript('OnClick', function(button)
			self:OnPersonalBankTabClick(button.tabIndex)
		end)
		...
		btn:Hide()
		self.personalBankTabs[i] = btn
	end

	local buy = CreateFrame("Button", nil, tabBar)
	buy:SetSize(TAB_SIZE, TAB_SIZE)
	buy:SetNormalTexture([[Interface\Buttons\UI-PlusButton-Up]])
	buy:SetPushedTexture([[Interface\Buttons\UI-PlusButton-Down]])
	buy:SetHighlightTexture([[Interface\Buttons\UI-PlusButton-Hilight]], "ADD")
	buy:SetScript('OnClick', function()
		if BuyGuildBankTab then
			BuyGuildBankTab()
		end
	end)
	buy:SetScript('OnEnter', function(button)
		GameTooltip:SetOwner(button, "ANCHOR_RIGHT")
		GameTooltip:SetText("Purchase tab")
		if GetGuildBankTabCost then
			local cost = GetGuildBankTabCost()
			if cost and GetCoinTextureString then
				GameTooltip:AddLine(GetCoinTextureString(cost), 1, 1, 1)
			end
		end
		GameTooltip:Show()
	end)
	...
	buy:Hide()
	self.PersonalBankBuyTab = buy
end

function containerProto:OnPersonalBankTabClick(tabIndex)
...
function containerProto:UpdatePersonalBankTabs()
	...
	for i = 1, MAX_PERSONAL_BANK_TABS do
		...
		if i <= numTabs then
			...
			btn:SetAlpha(isViewable == false and 0.4 or 1)
			btn:Show()
			lastVisible = btn
		else
			btn:Hide()
	...
	local buy = self.PersonalBankBuyTab
	if buy then
		if numTabs < MAX_PERSONAL_BANK_TABS and BuyGuildBankTab then
			if lastVisible then
				buy:SetPoint("TOPLEFT", lastVisible, "BOTTOMLEFT", 0, -TAB_SPACING)
			else
				buy:SetPoint("TOPLEFT", self.PersonalBankTabBar, "TOPLEFT", 2, 0)
			end
			buy:Show()
		else
			buy:Hide()
...
function containerProto:UpdateContent(bag)
	...
	if bag == PERSONAL_BANK_CONTAINER then
		local tab = GetCurrentGuildBankTab and GetCurrentGuildBankTab() or 1
		local newSize = MAX_GUILDBANK_SLOTS or 98
		content.family = 0
		content.guildTab = tab
		for slot = 1, newSize do
			local texture, count, locked = GetGuildBankItemInfo(tab, slot)
			local link = GetGuildBankItemLink(tab, slot)
			local itemId = link and tonumber(strmatch(link, "item:(%d+)")) or nil
			if not itemId or (link and IsValidItemLink(link)) then
				...
				local name, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, itemTexture, vendorPrice
				if link then
					name, _, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, itemTexture, vendorPrice = GetItemInfo(link)
					count = count or 0
					texture = texture or itemTexture
				else
					link, count, texture = false, 0, nil
				end
				...
				if GetDistinctItemID(slotData.link) ~= GetDistinctItemID(link) then
					...
					slotData.texture = texture
					slotData.locked = locked
					slotData.guildTab = tab
					slotData.name, slotData.quality, slotData.iLevel, slotData.reqLevel, slotData.class, slotData.subclass, slotData.equipSlot, slotData.vendorPrice = name, quality, iLevel, reqLevel, class, subclass, equipSlot, vendorPrice
					slotData.maxStack = maxStack or (link and 1 or 0)
					added[slotData.slotId] = slotData
				elseif slotData.count ~= count or slotData.locked ~= locked then
					slotData.count = count
					slotData.locked = locked
					slotData.texture = texture
